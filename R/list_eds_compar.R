list_eds_compar <- function(lgrph, var, common.eds.color = "red", different.eds.color = "orange", 
    common.eds.width = 2, different.eds.width = 1, verbose = F) {
    # verbose <- T ; var <- 'type'
    ...
    for (dec in 1:nrow(ldec.comp)) {
        ...
        # lgcomp <- list(lgrph[[to.compare[1]]],lgrph[[to.compare[2]]]) esss <-
        # igraph::intersection(lgcomp)
        ...
        # same.type <- igraph::E(common.edges)$type_1 ==
        # igraph::E(common.edges)$type_2
        common.edges.same.type <- igraph::E(common.edges)[same.type]
        # add attribute
        common.edges <- igraph::set_edge_attr(common.edges, "same", index = igraph::E(common.edges), 
            1)
        ab <- igraph::as_data_frame(common.edges)
        if (nrow(ab) > 0) {
            ab.same <- subset(ab, same == 1)
            ab.same <- subset.data.frame(ab.same, select = c("from", "to", 
                var.A))
        ...
        for (g in 1:length(grph.comp)) {
            # loop through A and B g <- 1
            grp <- grph.comp[[g]]
            g.sz <- igraph::gsize(grp)  # by edges
            for (e in 1:g.sz) {
                # loop through edges e <- 1
                ...
                for (c.e in 1:nrow(ab.same)) {
                  # loop through same edges c.e <- 1
                  same.ed.from <- ab.same[c.e, "from"]
                  same.ed.to <- ab.same[c.e, "to"]
                  same.ed.var <- ab.same[c.e, var]
                  # same.ed.from <- ab.same[c.e,]$from same.ed.to <- ab.same[c.e,]$to
                ...
                # common vs different
                if (flag.same) {
                  grp <- igraph::set_edge_attr(grp, "color", index = e, 
                    common.eds.color)
                  grp <- igraph::set_edge_attr(grp, "width", index = e, 
                    common.eds.width)
                  grp <- igraph::set_edge_attr(grp, "comm", index = e, 
                    1)
                } else {
                  grp <- igraph::set_edge_attr(grp, "color", index = e, 
                    different.eds.color)
