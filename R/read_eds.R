read_eds <- function(site, decor, doss = getwd(), edges = "edges", nodes = "nodes", 
    dev = ".tsv") {
    # doss <- doss.extdata ; edges <- 'edges' ; dev <- '.shp' doss <-
    # doss.extdata ; edges <- 'edges' ; dev <- '.tsv' nodes <- 'nodes' site
    # <- 'Cerro Muriano' ; decor <- 'Cerro Muriano 1' read nodes from .tsv
    # or shapefile TODO: read .csv
    ...
    if (dev == ".tsv") {
        ...
        eds.df <- utils::read.table(file = paste0(doss, "/", edges, dev), 
            sep = "\t", header = TRUE)
        ...
        for (a.edge in 1:nrow(eds.df)) {
            ...
            eds.shp.coords[nrow(eds.shp.coords) + 1, ] <- c(xa, ya, xb, 
                yb)
        }
