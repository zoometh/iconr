same_eds <- function(lgrph, var = "type", verbose = FALSE) {
    ...
    mat.same_edges <- matrix(nrow = length(lgrph), ncol = length(lgrph))
    ...
    for (i in 1:length(lgrph)) {
        ...
        if (verbose) {
            print(paste0("read graph: ", lgrph[[i]]$name))
        }
        # mat <- as_edgelist(lgrph[[i]]) i <- 4
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - df.typ_edges <-
    # f_typ_edges(tot.edges,lgrph,verbose = FALSE) # call private function
