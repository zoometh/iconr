...
#' @param out.data if "mbrshp" return a dataframe of nodes with their clustering and image path, if "plot" return a "kmeans" or create a plot

library(Momocs)

nds_morph_classif <- function(nodes,
  ...
  for(gu.type in gu.types){
    ...
    if("kmeans" %in% focus){
      ...
      if("plot" %in% out.data){
        ...
        dev.off()
        return(KMEANS(PCA.type, centers = nb.centers))
      }
