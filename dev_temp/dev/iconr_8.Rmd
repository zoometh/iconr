...
Its purpose is to contribute to cross-cultural iconography comparisons studies by a higher normalization of quantitative analysis [@HuetAlexander15]. This package has been grounded on the seminal work of C. Alexander [@Alexander08] and its first IT implementation by T. Huet [@Huet18a]. 
...
library(magick)
setwd(system.file("extdata", package = "decorr"))
plot(image_read(a.img <- imgs.df[1,"img"]))
```
...
Similarly to edges comparisons, by default the nodes in common are in <span style="color:red">red</span>, but their colors, and other graphical parameters, can be modified.
...
By default the edges in common are in <span style="color:red">red</span>, but their colors, and other graphical parameters, can be modified.
