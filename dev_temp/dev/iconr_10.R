...
if (pck.run){
  ...
  plot.dec.compar(g.compar,c(1,3))
}
if (pck.create){
...
if (fig.create){
  ...
  for(i in 1:4){
    lbl.txt <- paste0(imgs.df[i,"idf"],"\n",
                      ...
                      imgs.df[i,"decor"],"\n",
                      imgs.df[i,"img"],"\n",
                      image_info(i1)$width," x ",image_info(i1)$height," px")
    i1 <- magick::image_read(imgs.df[i,"img"])
    # i1 <- magick::image_annotate(i1,lbl.txt,location = "northwest",size = lbl.size)
    # i1 <- magick::image_annotate(i1,paste0(,"\n",imgs.df[i,"decor"]),gravity = "northeast",size = lbl.size)
    # i1 <- magick::image_annotate(i1,imgs.df[i,"img"],gravity = "southwest",size = lbl.size)
    # i1 <- magick::image_annotate(i1,paste(),
    #                              gravity = "southeast",
    #                              size = lbl.size)
    lims[[length(lims)+1]]<- i1
  }
  setwd("D:/Projet Art Rupestre/scripts/scripts_r/package_create/iconr/jss-article-rnw/imgs")
  out.img <- magick::image_append(c(c(lims[[1]],lims[[2]]),
                                    c(lims[[3]],lims[[4]])), stack = TRUE)
  out.img <- magick::image_append(c(I,II), stack = TRUE)
  # out.img <- magick::image_scale(out.img,"x400")
  magick::image_write(out.img, path = "tiger.png", format = "png")
}
# DATA  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
if (data.create){
