...
plot.dec.comp <- function(listg,graph2){
  # a manner to use only plot()
  # listg <- grphAllcompar ; df.compar <- ldec.comp ; graph2 <- c(1,3)
...
g.compar <- list.grph.compar(lgrph)
plot.dec.comp(g.compar,c(3,4))

list.grph.compar <- function(lgrph,common.eds.color="red",different.eds.color="orange",verbose=F){
  lidf <- unlist(lapply(lgrph, function(x) x$name))
  ldec.comp <- t(combn(lidf, 2)) # all pairwise comparisons
  if(verbose){
    ...
    for(g in 1:length(grph.comp)){
      ...
      for (e in 1:gsize(grp)){
        ...
        if(all(grp.ed.is %in% ab.same)){
          ...
          grp <- igraph::set_edge_attr(grp, "width", index = e, common.eds.color)
        }
        if(!all(grp.ed.is %in% ab.same)){
...
# ldec.comp <- t(combn(imgs$idf, 2)) # all pairwise comparisons
# print(paste0("there is ",nrow(ldec.comp)," pairwise comparisons to compute"))
# grphAllcompar <- list()
# for (dec in 1:nrow(ldec.comp)){
#   # dec <- 1
