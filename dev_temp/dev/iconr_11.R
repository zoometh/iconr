...
if (pck.run){
  ...
  g.compar <- list_compar(lgrph,var="type")
  ## plot
  nds.df <- read_nds(site = "Cerro Muriano",decor = "Cerro Muriano 1",dev = ".tsv")
  eds.df <- read_eds(site = "Cerro Muriano",decor = "Cerro Muriano 1",dev = ".shp")
  ...
  ## plot
  # edges
  # edges[44,"type"] <- "=" # a change to see..
  # lgrph <- list_dec(imgs, nodes, edges,var="type")
  # g.compar <- list_compar(lgrph, "type")
  compar_nds_1_4 <- plot_compar(g.compar,c(2,4),focus = "edges",doss= dataDir)
