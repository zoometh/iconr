...
%  instructions are marked by vertical bars.
  Pre- and Protohistorical societies are characterised by the absence of a writing system. Writing is known to be one of the most rational symbolic system with i) a clear distinction between signified and signifier -- specially in alphabetic and binary writings --, ii) the development of the signified on a horizonatl, vertical or boustrophedon axis. In illiterate societies, testimonies of symbolic systems mostly come from iconography (ceramic decorations, rock-art, statuary, etc.) and signs are displayed mostlty a discontinuous figures which can have different relationships one with another.

  To understand meaningful associations of signs, geometric tools, graph analysis and statistical analysis offer great tools to recognize iconographical patterns and to infer collective conventions. We present the \pkg{decorr} \proglang{R} package which ground methods and tools to analyse ancient iconographical systems.
}
...
\usepackage{Sweave}
\begin{document}
\input{article_rvTH-concordance}
...
\bibliography{D:/DocumentationArcheo/biblio8}
