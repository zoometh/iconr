...
\begin{figure}[H]
<<art_only, echo=FALSE, fig=TRUE, height=3>>=
...
\begin{figure}[H]
<<art_or_rat, echo=FALSE, fig=TRUE, height=3>>=
# library(extrafont)
# loadfonts(device = "win")
par(mar=c(0.1,0.1,0.1,0.1) )
ugs <- data.frame(name=c("a","r","t"))
...
\SweaveOpts{width=12,height=12}
<<echo=FALSE, fig=TRUE, fig.height=5>>=
library(magick)
...
lbl.size <- 25
lims <- list()
for(i in 1:nrow(imgs.df)){
  i1 <- image_read(paste0(pth,"\\",imgs.df[i,"img"]))
...
<<plotg, echo=TRUE, fig=TRUE, height=3,width=3>>=
plot(lgrph[[1]],
     vertex.size = 10,
     vertex.label.cex =.8)
@
...
<<plotcm, echo=TRUE, fig=TRUE, height=10,width=5>>=
# library(decorr)
nds.df <- read_nds(site = "Torrejon Rubio", decor = "Torrejon Rubio 1", dev = ".tsv",
                   doss = system.file("extdata", package = "decorr"))
eds.df <- read_eds(site = "Torrejon Rubio", decor = "Torrejon Rubio 1", dev = ".tsv",
                   doss = system.file("extdata", package = "decorr"))
...
<<plotedscomp echo=TRUE, fig=TRUE, height=14,width=14>>=
library(magick)
