...
\usepackage{Sweave}
\begin{document}
\input{article_rvTH10-concordance}
...
\centering
\includegraphics{article_rvTH10-art_only}
...
\begin{minipage}{0.8\textwidth}
\centering
\includegraphics{article_rvTH10-art_or_rat}
...
\begin{figure}[H]
\centering
\includegraphics{gis_1}
\caption{\label{fig:gis1} The graphical unit 'casque' (helmet) is an attribute of the graphical unit 'personnage'. (character).}
\end{figure}
...
\label{Test_table}
\end{table}
The default dataframe is \code{imgs}. It list the decorations. The field \code{imgs$idf} is the short name of the decoration. The concatened of  \code{imgs$site} and \code{imgs$decor} is the key of the decoration. Decoration images are All the drawings belongs to \citep{DiazGuardamino10}.

\begin{figure}[H]
\centering
\includegraphics{tiger}
\caption{\label{fig:tiger} Dataset of four stelae showing the information on the decoration (\code{site}, \code{decor} and \code{img}) and the dimensions in pixel of the decoration image}
\end{figure}
...
> # plot the first graph of the list
> g <- lgrph[[1]]
> plot(g,
+      vertex.size = 10,
+      vertex.label.cex =.8)
\end{Sinput}
\end{Schunk}
\includegraphics{article_rvTH10-geomg}
