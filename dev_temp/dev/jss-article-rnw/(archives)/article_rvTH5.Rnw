...
In this article we present the \proglang{R} package \pkg{decorr}. Its purpose is to offer explicit concepts, methods and indexes to analyse a graphical content in order to make possible cross-cultural comparisons. 
...
Graph theory (i.e. network analysis) offers a vocabulary and a conceptual framework to deal with notions of networks, relationships and neighbourhoods. A system can be represented by vertices connected (or not) to each other with edges. Let us take the example of the word "\code{art}" contains three vertices (\code{a}, \code{r}, \code{t}) and two edges (between \code{a} and \code{r}, between \code{r} and \code{t}).
