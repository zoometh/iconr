...
\usepackage{Sweave}
\begin{document}
\input{article_rvTH7-concordance}
...
As any formal system, iconography can be seen as spatial features related one with the other depending on rules of proximities. Just like in \proglang{R}, the features \code{a}, \code{r} and \code{t} concatenated in this order mean "\code{art}", and not "\code{rat}", in illetrate societies, spatial relations between graphical features are not necessarly linear and directed but multi-directional and undirected: the direction of the interactions of pairwise graphical units can be in any order (Fig. \ref{fig:triangle})
...
\end{tikzpicture}
\caption{Potential spatial relations between \code{a}, \code{r} and \code{t} graphical units. Edges of the triangle represent the proximity links between the \code{a}, \code{r} and \code{t} graphical features} \label{fig:triangle}
\end{figure}
...
\code{list_nds_compar} and \code{list_eds_compar} allow to compare respectively the common nodes and the common edges between two graphs
