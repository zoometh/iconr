...
library(iconr)
# After unzipped
dataDir <- "C:/Users/supernova/Dropbox/My PC (supernova-pc)/Downloads/Valcamonica/Valcamonica"
# Read data
imgs <- read.table(paste0(dataDir, "/imgs.tsv"),
                   sep="\t",stringsAsFactors = FALSE)
nodes <- read.table(paste0(dataDir, "/nodes.tsv"),
                    sep="\t",stringsAsFactors = FALSE)
edges <- read.table(paste0(dataDir, "/edges.tsv"),
                    sep="\t",stringsAsFactors = FALSE)
...
plot_compar(g.compar, c(1:3), 
            nd.color = c("orange", "red"), nd.size = c(0.5, 3),
            ed.color = c("orange", "red"), ed.width = c(1, 3),
            lbl.size = 0.5,
            focus = "edges",
            dir = dataDir)
```
