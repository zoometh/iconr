---
title: "**The R *iconr* package** <br> GIS interface entry"
...
Any iconographic contents can be modeled with a geometric graph where nodes, also called **graphical units (GUs)**, linked together with edges and then analyzed with the **Graph Theory** and spatial analysis at the support scale. T
...
These datasets can be downloaded by clicking on their names. A RShiny app allows to [interactively plot these datasets](https://epispat.shinyapps.io/decoration_graph/) with the `plot_dec()` function
...
The **table of decorations** is a correspondance table which records joins between nodes and edges dataframes. In the package, the default name of this table is `imgs.tsv`, a tabulate separated-values (but it also can be a`.csv`, comma separated-values)
