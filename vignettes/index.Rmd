...
```{r gis, out.width="60%", echo=FALSE, fig.align='center', fig.cap="GIS view of the Cerro Muriano 1 stelae with the Voronoi cells of its GUs, GUs (main nodes) and proximity links between these GUs (edges)"}
...
* **[Table of decorations](#decorations):** Tabular file storing the set of decoration identifiers and corresponding image filenames.
...
The `iconr` package includes an example dataset with the input files in several alternative formats. The path for the example dataset is the package *exdata* folder:
...
Each decoration is identified by its name and the name of the site to which it belongs. In the example dataset, this is transparent in the name of each decoration image, included in jpeg format. Any other image format supported by the R package `magick` (jpeg, png, tiff, pdf, etc.) is suitable.

A GIS interface is often the most practical way to record graph nodes and graph edges with `POINTS` and `LINES` geometries, respectively. This is typically saved in shapefile (shp) format, which is composed of at least 3 files with extensions `.shp`, `.shx`, and `dbf`. The example dataset includes them for nodes and edges separatelly, with obvious names:
...
By default, the `plot.igraph()` function (ie, `igraph::plot()`) spatialisation (`layout`) is based on `x` and `y` columns, when these exist. This is appropriate in our case since we are working with geometric graphs. If this spatialisation (node coordinates) is ignored, then very different layouts are possible for the same graph (*graph drawing*):
...
in any compatible file format readable by R, such as comma-separated-value (csv).

Their identifiers and image filenames are stored in the `imgs` dataframe:
...
For a given decoration, its image is the reference space of the graph: nodes and edges inherit their coordinates from the image grid. The package `iconr` considers the following coordinate conventions:
...
```{r drawing, out.width="50%", fig.align="center", warning=FALSE, echo=TRUE, message=FALSE, fig.cap="\\label{fig:figs}Cerro_Muriano.Cerro_Muriano_1.jpg with the coordinates of its corners following the package `iconr` convention."}
library(magick)
dataDir <- system.file("extdata", package = "iconr")
imgs_path <- paste0(dataDir, "/imgs.csv")
imgs <- read.table(imgs_path, sep=";", stringsAsFactors = FALSE)
...
Nodes can be stored in any of the following formats: comma-separated-value (`.csv`), tab-separated-value (`.tsv`), or shapefile (`.shp`). Any of these formats is consistenly read by the function `iconr::read_nds` generating a data.frame with the node data. For the generic csv and tsv formats, at least the following five columns are required to be included: 
...
These node characteristics can then be used for image annotations and decoration comparisons and analysis.
...
In principle, the nodes are defined to be located at the exact centroids of each GU. However, they can also be manually located near the centroids.
...
Those are imported from the `x` and `y` coordinates in the corresponding [node](#nd) file, identified by the node `id` as stated in the edge fields `a` and `b`, respectively.
...
```{r graph.normal.plot.type,  out.width="100%", fig.width=16, fig.asp=0.55, fig.align="center", warning=FALSE, fig.cap="Brozas stelae (decoration 3) with only *normal* edges: all the composition elements seem contemporaneous"}
site <- "Brozas"
decor <- "Brozas"
...
```{r graph.attribute.plot.type, out.width="100%", fig.width=16, fig.asp=0.55, fig.align="center", warning=FALSE, fig.cap="Zarza De Montanchez stelae (decoration 4) showing *normal* and *attribute* edges"}
...
plot_dec_grph(nds.df, eds.df, imgs,
              site, decor, dataDir,
              ed.lwd = 2, ed.color = c("darkorange"),
              lbl.size = 0.8,
              img.format = NULL)
...
```{r graph.overlap.plot.type, out.width="100%", fig.width=16, fig.asp=0.55, fig.align="center", warning=FALSE, fig.cap="Ibahernando stelae (decoration 5) showing *normal* and *diachronic* edges"}
site <- "Ibahernando"
decor <- "Ibahernando"
nds.df <- read_nds(site, decor, dataDir)
eds.df <- read_eds(site, decor, dataDir)
...
nds.df <- read_nds(site, decor, dataDir)
eds.df <- read_eds(site, decor, dataDir)
img.graph <- plot_dec_grph(nds.df, eds.df, imgs,
                           site, decor, dataDir,
                           nd.var = 'type',
                           lbl.size = 0.55,
                           img.format = NULL)
...
Regarding the node variable `type`, the decoration 4 has twelve (12) nodes, has nine (9) common nodes with the decoration 2, and has four (4) common nodes with the decoration 3. This matrix can be used for further [clustering analysis](#sum.class)
...
l_dec_df <- contemp_nds(nds.df, eds.df, selected.nd)
img.graph <- plot_dec_grph(l_dec_df$nodes, l_dec_df$edges, imgs,
