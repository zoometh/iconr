...
The `iconr` package includes an example dataset with the input files in several alternative formats. The path for the example dataset is the package *extdata* folder:
...
Different *main* nodes considered to be contemporaneous and close to one another, may share an edge with the  value `=` for their `type`. By convention, these edges are called *normal* and displayed as a plain line.
