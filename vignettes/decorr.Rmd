...
- id: Toussaint80
  ...
  - family: Toussaint
    given: G. T.
  container-title: "Pattern recognition 12(4)"
  type: article-journal
  page: 261-268
...
The R package `decorr` is grounded on graph theory and spatial analysis. It offers concepts and functions to model Prehistorical iconographical composition and to prepare further analysis (clustering, Harris diagram, etc.) in oreder to contribute to cross-cultural iconography comparisons studies by a higher normalization of quantitative analysis [@Alexander08; @HuetAlexander15; @Huet18].  

The main principle of `decorr` package is to considerate any iconographic compositions (here, 'decorations') as relative neighborhood graphs (RNG) of graphical units (GUs). The GUs are decorated surfaces (`POLYGONS`) modelised as nodes (`POINTS`) sharing a proximity link one with another (edges) when their Voronoi cell share a border. 
&nbsp;
  
  
```{r gis, out.width="33%", echo=F, fig.align='center', fig.cap="GIS view of the Cerro Muriano 1 stelae with the Voronoi cells of its GUs, GUs (nodes) and proximity links between these GUs (edges)"}
...
Functions are provided to manage [nodes (`nds`)](#nd), edges [(`eds`)](#ed), [graphs](#grph) with the basic requirements to [plot](#plot) and [compare](#compare) them
...
```{r img.graph.plot.type, fig.width=6, fig.height=6, fig.align="center", warning=FALSE, fig.cap="Cerro Muriano 1 drawing with the type of each GU"}
...
```{r img.graph.plot.id, fig.width=6, fig.height=6, fig.align="center", warning=FALSE, fig.cap="Cerro Muriano 1 drawing with the maximum length (in cm) of each GU"}
nds.df <- read_nds(site = "Cerro Muriano", decor = "Cerro Muriano 1", doss = dataDir)
...
## Compare decoration graphs {#compare}
...
* cells of the dataframe show the total number of common edges by decorations  
...
## Contemporaneous iconographical contents {#contemp}

It occurs that some GUs are non-contemporaneous one with the others, like for the latin writing overlapping Late Bronze Age engravings on Ibahernando stelae. In this case, the non-contemporaneous layers of decoration should be removed before the comparison process. To do so, the original graph (1-component) will be splitted into different sub-graphs (n-component) by selecting `->-` edges (see [edge types](#ed.type)). The studied graph component will be retieved with the component membership of a selected node in the `contemp_nds()` function parameters.  

For example, the Ibahernando stelae have been found reused in a Roman funerary context. A latin inscription have been carved over a spear (lance, node 2) and overlaps partially a V-notched shield (bouclier, node 3). To study only the Late Bronze Age iconographic layer, we can choose the Late Bronze Age node 4, the figure of sword (epee) dated to the middle and final stages of Late Bronze Age (ca 1250-950 BC)

```{r rm.writing, out.width="100%", fig.align="center", warning=FALSE, fig.cap="Ibahernando before and after the suppression of the overlapping node"}
selected.nd <- 4
...
Ibahernando.contemp <- plot_dec_grph(nodes = l_dec_df[[1]],
                                     ...
                                     lbl.size=2.2,
                                     doss = dataDir)
magick::image_append(c(Ibahernando, Ibahernando.contemp))
```
&nbsp;
...
## Graph decoration {#grph}

The decoration graph is constructed from the [nodes data](#nd) (`.csv`, `.tsv` or `.shp`) and the [edges data](#ed) (`.csv`, `.tsv` or `.shp`). As a RNG graph, the graph is 1-component: the graph covers all the GUs of the decoration
...
As state by the graph theory:

* **undirected edges**: *normal* edge (`1-=-2`) is equal to (`2-=-1`)  

* **directed edges**: 
  * *attribute* edges (`1-+-2`) is not equal to (`2-+-1`)
  * *overlap* edges (`1->-2`) is not equal to (`2->-1`) 
  
By default: 

* all comtemporaneous nodes have *normal* or *attribute* edges displayed in <span style="color:orange"><b>orange</b></span>

* all non-comtemporaneous have an *overlap* edge and displayed in <span style="color:blue"><b>blue</b></span> edges (see [contemporaneous nodes](#contemp)). 

***normal* edges**

Usually, differents nodes considerated as contemporaneous and close one with another, shared an edge with the  value `=` (textual notation: `-=-`) for their `type`. By convention, these edges are called *normal* and displayed with a plain line.

```{r graph.normal.plot.type, fig.width=6, fig.height=6, fig.align="center", warning=FALSE, fig.cap="Brozas decoration with only *normal* edges: all the composition seems contemporaneous"}
...
When a node is an attribute of another, edges are identified with a `+` (textual notation: `-+-`) and displayed with a dashed line.  For example, at the bottom of the `Zarza De Montanchez` decoration, the main GU chariot `7` is connected with four nodes attributes:

- two horses (cheval): `8` and `9`  
- two wheels (roue): `10` and `11` 

```{r graph.attribute.plot.type, out.width="100%", fig.align="center", warning=FALSE, fig.cap="Zarza De Montanchez decoration, *normal* and *attribute* edges"}
...
img.graph.type <- plot_dec_grph(nodes = nds.df,
                                ...
                                nds.lbl = 'type',
                                doss = dataDir)
magick::image_append(c(img.graph.id,img.graph.type))
```
&nbsp;
...
When a node overlaps another -- or is more recent than another -- edges are identified with a `>` (textual notation: `->-`) and displayed with a blue plain line. For example, the `Ibahernando` stelae show a latin writing (écriture) overlapping a spear (lance) and a shield (bouclier).


```{r graph.overlap.plot.type, out.width="100%", fig.align="center", warning=FALSE, fig.cap="Ibahernando decoration, *normal* and *overlap* edges"}
nds.df <- read_nds(site = "Ibahernando", 
                   decor = "Ibahernando",
                   doss = dataDir)
eds.df <- read_eds(site = "Ibahernando",
