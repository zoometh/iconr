...
The training dataset are [drawings with their listing](#drawings) and data for [nodes](#nd), [edges](#ed). The current path of training dataset is the *exdata/* folder. All the *input* data (dataframes, shapefiles, images) start with a letter (`"^[[:alpha:]]"`) while the *output* data start with a punctuation (`"[[:punct:]]"`) or a numerical (`"[[:digit:]]"`)
...
+ images (common formats: `jpg, png, jpeg, tiff, pdf,` etc.)
...
Since a GIS interface is often the most practicale to record graph nodes and graph edges with `POINTS` and `LINES` geometries coming from shapefiles (`.shp`). Nodes and edges can also be recorded in tabular format: `.tsv` (tab-separated values) or `.csv` (semicolons-separated values)   
...
The list of graph decorations is created with the `list_dec()` function

```{r listdec, results='asis', warning=FALSE}
lgrph <- list_dec(imgs, nodes, edges)
plot(lgrph[[1]])
```

These graphs are `igraph` objects

```{r graph.clss}
as.character(class(lgrph[[1]]))
```
...
```{r drawing,fig.width=6, fig.height=6, fig.align="center", warning=FALSE, echo=TRUE, message=FALSE, fig.cap="\\label{fig:figs}Cerro_Muriano.Cerro_Muriano_1.jpg with the coordinates of its corners"}
library(magick)
dataDir <- paste0(system.file("extdata", package = "decorr"))
imgs_path <- paste0(dataDir, "/imgs.csv")
imgs <- read.table(imgs_path, sep=";", stringsAsFactors = FALSE)
...
as.character(named_elements(lgrph[[1]], focus = "edges", nd.var="type"))[1]      
```

Employed with the basic R functions for loop (`lapply()`), counts (`table()`) oand order (`order()`), this function can be used to count the different types of edges. Here, the most represented ones:

```{r count.all, warning=FALSE}
all.edges <- lapply(lgrph, function(x) as.character(named_elements(x, focus = "edges", nd.var="type")))
all.edges.ct <- as.data.frame(table(unlist(all.edges)))
...
```{r graph.normal.plot.type,  out.width="100%", fig.align="center", warning=FALSE, fig.cap="Brozas decoration with only *normal* edges: all the composition seems contemporaneous"}
...
Regarding the node `type` variable, the decoration 4 has twelve (12) nodes, has nine (9) common nodes with the decoration 2, and has four (4) common nodes with the decoration 3. To compare the decorations 2, 3 and 4 on the `type` variable:
...
It occurs that some nodes are non-contemporaneous one with the others, like for the Ibahernando stelae. This stelae have been found reused as a funerary steale during Roman times [@Almagro66b]. 

<center>

![GIS view. The Ibahernando stelae](`r ibahernando.path`){width=400px}

</center>

A latin inscription (ecriture, node 1) have been carved over a spear (lance, node 2) and overlaps partially a V-notched shield (bouclier, node 3). The edges between node 1 and node 2, and the edge between node 1 and node 3, are *overlap* edges
...
At the opposite, epigraphists will study only the iconographic layer of the latin writing
...
Beside graphical functions to highlight common elements (nodes and edges) between decorations. The package allows to classify these decorations by statistical similarities.

```{r clust.comp, warning=FALSE, out.width="100%"}
par(mfrow=c(1, 2))
...
These classifications can be compared one to another

```{r hclust.compar, warning=FALSE, out.width="100%"}
...
![GIS view. The Brozas stelae](`r brozas.path`){width=400px}

</center>

## Nodes tree

Graph theory permit to construct tree structures and to include processes of ***generalization*** (parent level) and ***specifications*** (children level) within the controled vocabulary. For example, the swords and spears belongs to the `weapons` group (sub-group `offensive weapons`), the shield belongs to the `weapons` group (sub-group `defensive weapons`), etc. 

```{r hierac, warning=FALSE, out.width="100%"}
