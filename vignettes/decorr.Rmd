...
When a GU overlapping another -- or is more recent than antother -- the edge is identified with a `>` and are displayed with a blue plain line. For example, the `Ibahernando` stelae show a latin writing (écriture) overlaping a spear (lance) and a shield (bouclier).
...
It occurs that some GUs are non-contemporaneous one with the others, like for the latin writing overlapping Late Bronze Age engravings on Ibaherando stelae. In this case, the non-contemporaneous layers of decoration can be removed before the comparison processes
