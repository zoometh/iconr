...
External data attempt give a training dataset to illustrate the package functions.  
...
The decorations unique identifiers are the concatenation of the name of the site and the name of the decoration. for example, the name of the `Cerrano Muriano 1` decoration is ``r imgs[1,"img"]``.  
...
library(magick)
plot(image_read(paste0(iconr_path, "/", imgs[1,"img"])))
```
...
Commonly differents GUs close one with another have the value `=` for their edge `type` and are displayed in <span style="color:orange">orange</span> and a plain line
...
When a GU is an attribute of another, the edge is identified with a `+` and are displayed with a dashed line.  

For example, at the bottom of the `Zarza De Montanchez` decoration, one (1) main GU, the chariot `7`, connected with four (4) attributes (or secondary GUs):
