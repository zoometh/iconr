...
Since a GIS interface is often the most practicale to record nodes and edges, the package has been designed to accept shapefiles (`.shp`) for nodes (`POINTS` geometries) and edges (`LINES` geometries).  
...
By default, all edges are displayed in <span style="color:orange"><b>orange</b></span>. Except the *normal* edges which is undirected, the *attribute* and *overlap* edges are directed.
...
```{r graph.normal.plot.type, fig.width=6, fig.height=6, fig.align="center", warning=FALSE, fig.cap="Brozas decoration"}
...
```{r graph.attribute.plot.type, fig.width=6, fig.height=6, fig.align="center", warning=FALSE, fig.cap="Zarza De Montanchez decoration"}
...
When a GU overlapping another, the edge is identified with a `>` and are displayed with a XXXX line. For example, the `Ibahernando` stelae show a latin writing overlaping a spear (lance) and a shield (bouclier).
  

```{r graph.overlap.plot.type, fig.width=6, fig.height=6, fig.align="center", warning=FALSE, fig.cap="Ibahernando decoration"}
...
# setwd(system.file("extdata", package = "decorr"))
nds.df <- read_nds(site = "Zarza de Montanchez", decor = "Zarza De Montanchez", doss = iconr_path)
...
Replot the graph with `id` field instead of the `type` field with bigger and <span style="color:orange">orange</span> labels
