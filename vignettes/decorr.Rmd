---
title: "The ***iconr*** package. Graph Analysis of Prehistorical Iconography with R"
...
The last development version of `iconr` package and its vignette can be download from GitHub

```{r down,eval=FALSE, echo=TRUE}
devtools::install_github("zoometh/iconr", build_vignettes=TRUE)
```

The R package `iconr` is composed by [functions](#functions) and a [dataset example](#data). The main R packages used by `iconr` package are:
...
As stated by the graph theory, edges can be undirected or directed. In the `iconr` package, by default: 
...
* all non-comtemporaneous nodes have [*diachronic* edges](#ed.type.over) and displayed in <span style="color:blue"><b>blue</b></span> edges (see [contemporaneous nodes](#contemp))
...
These overlappings, or diachronic iconographical layers, can be manage with the `contemp_nds()` function (see the section [Contemporaneous contents](#contemp))

# Functions {#functions}

Decoration graphs are constructed from nodes and the edges. As a RNG graph, graphs are 1-component: each decoration graph covers all the GUs of the decoration. The functions of the `iconr` package provide basic requirements to manage these [nodes data](#nd) (`.csv`, `.tsv` or `.shp`) and [edges data](#ed) (`.csv`, `.tsv` or `.shp`) to create graphs, to [plot](#plot) and to [compare](#compare) them, to select [contemporaneous GUs compositions](#contemp)
...
Plasticity of graph theory and facilities coming from the GIS database make `iconr` package usefull to manage, plot and compare large series of iconographical contents.  
